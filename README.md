...
您可以透過設定小工具的參數 (Parameter) 來決定要顯示什麼資訊：

1. 長按桌面上的 FIDP Lite 小工具，選擇【編輯小工具】。
2. 在 `Parameter` 欄位中：
   * **追蹤特定航班**：輸入航班號碼（例如：`CX840` 或 `UO132`）。請盡量確保與機場官方格式一致。
   * **隨機航班**：留空不填，系統將自動隨機抽取一班近期的航班顯示。
   * **離線測試模式**：輸入 `OFF`，可載入一筆靜態測試數據。
3. 點擊空白處完成編輯。

## ⚠️ 重要注意事項
* **iOS 系統限制 (非絕對即時)**：受限於 iOS 對桌面小工具背景重新整理頻率的嚴格限制，Widget 上顯示的數據 **並非絕對的實時 (Real-Time) 數據**。
* **更新頻率**：腳本預設要求每 3 分鐘更新一次，但實際刷新時間取決於 iOS 系統動態分配的資源。
* **使用建議**：在使用航班資訊（特別是登機閘口或最後召集等緊急資訊）前，**請務必核對小工具左下角的「最後更新時間」**。您可以直接點擊 Widget 來手動觸發腳本重新執行並更新數據。
